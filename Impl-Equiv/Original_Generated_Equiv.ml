(**
 ...
 File        : Original_Generated_Equiv.ml
 Authors     : Washington Lu\'is R. de Carvalho Segundo and
               Mauricio Ayala Rinc\'on 
               Universidade de Bras\'ilia (UnB) - Brazil
               Group of Theory of Computation
 
 Description : This file was automatically generated from the 
               file AACC_Equiv_rec.v of the specification, via 
               Coq code extraction.
...
let rec tPithdel i t e n =
  ...
  | Pr (t1, t2) ->
    ...
    (match le_dec i l1 with
     | Left ->
       (match eq_nat_rec l1 (S O) with
        | True -> t2
        | False -> Pr ((tPithdel i t1 e n), t2))
     | Right ->
       let ii = sub i l1 in
       (match eq_nat_rec l2 (S O) with
        | True -> t1
        | False -> Pr (t1, (tPithdel ii t2 e n))))
  | Fc (e0, n0, t0) ->
    (match eq_nat_rec (tPlength (Fc (e0, n0, t)) e n) (S O) with
     | True -> Ut
          ...
          (match t0 with
           | Fc (n1, n2, t2) ->
             (match match eq_nat_rec n0 n2 with
                    | True -> eq_nat_rec O n1
                    | False -> False with
              | True ->
                (match equiv_rec0 c0 (tPith (S O) t1 O n0)
                         (tPith (S O) t2 O n0) __ with
                 | True ->
                   equiv_rec0 c0 (tPithdel (S O) (Fc (O, n0, t1)) O n0)
                     (tPithdel (S O) (Fc (O, n0, t2)) O n0) __
                 | False -> False)
              | False -> False)
           | _ -> False)
             ...
             (match t0 with
              | Fc (n2, n3, t2) ->
                (match match eq_nat_rec n0 n3 with
                       | True -> eq_nat_rec (S O) n2
                       | False -> False with
                 | True ->
                   let branch = fun i ->
                     match equiv_rec0 c0 (tPith (S O) t1 (S O) n0)
                             (tPith i t2 (S O) n0) __ with
                     | True ->
                       equiv_rec0 c0
                         (tPithdel (S O) (Fc ((S O), n0, t1)) (S O) n0)
                         (tPithdel i (Fc ((S O), n0, t2)) (S O) n0) __
                     | False -> False
                   in
                   iter branch (S O) (tPlength t2 (S O) n0)
                 | False -> False)
              | _ -> False)
           ...
           | S n2 ->
             (match n2 with
                      ...
                      (match t4 with
                       | Pr (t5, t6) ->
                         (match match eq_nat_rec n0 n4 with
                          ...
                          | False -> False)
                       | x0 ->
                         (match match eq_nat_rec (S (S O)) n3 with
              ...
              | S n3 ->
                (match t0 with
                 | Fc (n4, n5, t2) ->
                   (match match eq_nat_rec (S (S (S n3))) n4 with
